...
ENVIRONMENT_VARIABLE = "ETODO_ENV"
...
DATABASES = {
    "default": {
        key.upper(): json.loads(value) if key == "options" else value
        for key, value in CNF._sections["DEFAULT DATABASE"].items()  # type: ignore
    }
}
...
LOGGING = {
    ...
    "root": {
        "handlers": ["etodo_handler"],
        "propagate": True,
    },
    "loggers": {
        "incident_management_logger": {
            "handlers": ["etodo_handler"],
            "level": "DEBUG",
        }
    },
...
CELERY_BROKER_URL = CNF.get("DEFAULT", "BROKER_URL")
CELERY_BACKEND = CNF.get("DEFAULT", "BROKER_URL")
